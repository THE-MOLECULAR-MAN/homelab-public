#!/bin/bash
# Tim H 2019
# This script creates a short-lived VPN server hosted in AWS designed to
# be used for just one session before the EC2 insance is terminated.
# It's designed for high security since the VPN server is pretty ephemeral
# It relies on an existing cloudformation template and will list
# the randomly created connection information when it is done
##############################################################################
#
#	References:
...
# adding a prefix so you can't get confused about which one is which
VPN_USERNAME=$(echo -n "u" && randpw)  # must start with letter
